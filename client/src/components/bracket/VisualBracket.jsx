        ...
        <div className="match-visual">
            ...
            <div className={"players"}>
                {Array.from({ length: bracketInfo.players_per_station }, (_, index) => (
                    <div className={"player-data match-player-" + (index < matchDetails.players.length ? 
                        (matchDetails.players[index]): matchDetails.is_bye ? ("null bye") : ("null"))} key={index}>
                        <div className="player-seed">
                            {index < matchDetails.players.length ? (
                                playerDetails.find(player => player.player_id === 
                                matchDetails.players[index]).seed + 1
                            ) : ""}
                        </div>
                        <div className="player-name">
                            {index < matchDetails.players.length ? (
                                playerDetails.find(player => player.player_id === 
                                matchDetails.players[index]).name
                            ) : matchDetails.is_bye ? (
                                "BYE"
                            ) : null}
                        </div>
